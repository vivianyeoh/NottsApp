...
public class MaintainCarDBTable {
                        ...
                        if (response.length() > 0 && response != null) {
                            try {
                                ...
                                for (int i = 0; i < response.length(); i++) {
                                    ...
                                    car = gson.fromJson(courseResponse.toString(), Car.class);
                                    carList.add(car);
                                    Log.wtf(TAG, "Car Data :" + car.toString());
                                }
    ...
    public void updateCar(final Car car) {
        ...
        try {
            StringRequest postRequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Log.wtf(TAG, "Car is updated " + response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
    ...
    public void deleteCar(final int id) {
        RequestQueue queue = Volley.newRequestQueue(context);
        String url = "http://notts.esy.es/delete_car.php";
